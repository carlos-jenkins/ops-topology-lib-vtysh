...
that will match the ``unknown command`` output. To specify a new typed exception
edit the file ``tools/vtysh_meta.py`` and modify the ``VTYSH_EXCEPTIONS_SPEC``
dictionary mapping the name of the new exception and a list of regular
expressions for that error that will be used to try to identify the output of a
command::

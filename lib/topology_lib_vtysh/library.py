...
def show_interface(enode, portlbl):
    ...
    :return: :py:func:`topology_lib_vtysh.parser.parse_show_interface`
    """
    if portlbl not in enode.ports.keys():
        msg = 'Unknown portlbl, available portlbl are: {}'.format(
              ', '.join('\'{}\''.format(k) for k in enode.ports.keys()))
...
def show_vlan(enode):
    ...
    :return: :py:func:`topology_lib_vtysh.parser.parse_show_vlan`
    """
...
def show_lacp_interface(enode, portlbl):
    ...
    :param portlbl: Label that identifies interface.
    :return: :py:func:`topology_lib_vtysh.parser.parse_show_lacp_interface`
    """
    if portlbl not in enode.ports.keys():
        msg = 'Unknown portlbl, available portlbl are: {}'.format(
              ', '.join('\'{}\''.format(k) for k in enode.ports.keys()))
...
def show_lacp_aggregates(enode, lag=''):
    ...
    :return: :py:func:`topology_lib_vtysh.parser.parse_show_lacp_aggregates`
    """
...
def show_lacp_configuration(enode):
    ...
    :return: :py:func:`topology_lib_vtysh.parser.parse_show_lacp_configuration`
    """
...
def show_lldp_neighbor_info(enode, portlbl):
    ...
    :param portlbl: Label that identifies interface.
    :return: :py:func:`topology_lib_vtysh.parser.parse_show_lldp_neighbor_info`
    """
    if portlbl not in enode.ports.keys():
        msg = 'Unknown portlbl, available portlbl are: {}'.format(
              ', '.join('\'{}\''.format(k) for k in enode.ports.keys()))
...
def show_lldp_statistics(enode):
    ...
    :return: :py:func:`topology_lib_vtysh.parser.parse_show_lldp_statistics`
    """
